...
import HappyDashboard from './components/HappyDashboard';
...
function App() {
    ...
    const [systemAuthRequest, setSystemAuthRequest] = useState(null); // { actions: [...] } â€” one-time startup auth
    ...
    const [lastActiveTime, setLastActiveTime] = useState(null); // Last time HAPPY responded
    ...
    useEffect(() => {
        ...
        socket.on('transcription', (data) => {
            // Track last active time when HAPPY/ADA sends a message
            const senderLower = (data.sender || '').toLowerCase();
            if (senderLower === 'happy' || senderLower === 'ada') {
        ...
        // One-time system authorization request on session start
        socket.on('system_auth_request', (data) => {
            console.log("[Auth] System auth request received:", data);
            setSystemAuthRequest(data);
        });

        // Response confirmation (clear dialog on response)
        socket.on('system_auth_response', (data) => {
            console.log("[Auth] System auth response:", data);
            setSystemAuthRequest(null);
        });

        // Voice-controlled camera & hand gesture mode
        socket.on('frontend_control', (data) => {
            console.log('[FE CONTROL] Action:', data.action);
            const { action } = data;
            if (action === 'camera_on') {
                setIsVideoOn(true);
            } else if (action === 'camera_off') {
                setIsVideoOn(false);
            } else if (action === 'hand_gesture_on') {
                setIsHandTrackingEnabled(true);
            } else if (action === 'hand_gesture_off') {
    ...
    return (
        ...
        >
            ...
            {/* â”€â”€ One-Time System Authorization Dialog â”€â”€ */}
            {systemAuthRequest && (
                <div className="hd-modal-overlay">
                    <div className="hd-modal-card" style={{ width: 320 }}>
                        <div style={{ padding: '28px 24px', textAlign: 'center' }}>
                            <div style={{ fontSize: 28, marginBottom: 12 }}>🛡️</div>
                            <div style={{ color: '#fff', fontWeight: 600, fontSize: 15, letterSpacing: '0.05em', marginBottom: 6 }}>
                                System Access Required
                            </div>
                            <div style={{ color: 'rgba(0,229,255,0.45)', fontSize: 11 }}>
                                One-time permission for this session
                            </div>
                        </div>
                        <div style={{ padding: '0 20px 24px', display: 'flex', gap: 10 }}>
                            <button
                                onClick={() => { socket.emit('authorize_system', { granted: false }); setSystemAuthRequest(null); }}
                                style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid rgba(100,100,120,0.35)', background: 'transparent', color: 'rgba(200,220,255,0.4)', fontSize: 13, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit' }}
                            >Deny</button>
                            <button
                                onClick={() => { socket.emit('authorize_system', { granted: true }); setSystemAuthRequest(null); }}
                                style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid rgba(0,229,255,0.4)', background: 'rgba(0,229,255,0.09)', color: '#00e5ff', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit', boxShadow: '0 0 16px rgba(0,229,255,0.1)' }}
                            >Allow</button>
                        </div>
                    </div>
                </div>
            ...
            {isVideoOn && isHandTrackingEnabled && (
                <div
                    style={{
                        ...
                        pointerEvents: 'none',
                        zIndex: 999,
                        boxShadow: isPinching ? '0 0 16px rgba(0,229,255,0.8)' : '0 0 8px rgba(0,229,255,0.3)',
                        background: isPinching ? 'rgba(0,229,255,0.3)' : 'transparent',
            ...
            {/* â”€â”€ Video PiP â”€â”€ */}
            <div className={`hd-video-pip ${isVideoOn ? '' : 'hidden'}`}>
                <div className="hd-cam-label">CAM_01</div>
                <video ref={videoRef} autoPlay muted style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0 }} />
                <canvas
                    ref={canvasRef}
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.85, transform: isCameraFlipped ? 'scaleX(-1)' : 'none' }}
                />
            </div>
            ...
            {showCadWindow && (
                <div
                    className="hd-overlay-win"
                    style={{
                        left: elementPositions.cad?.x || '50%',
                        top: elementPositions.cad?.y || '50%',
                        transform: 'translate(-50%,-50%)',
                        width: elementSizes.cad.w,
                        height: elementSizes.cad.h,
                        zIndex: getZIndex('cad'),
                    }}
                    onMouseDown={e => handleMouseDown(e, 'cad')}
                >
                    <div className="hd-overlay-header" data-drag-handle>
                        <span className="hd-overlay-title">CAD PROTOTYPE</span>
                        <button className="hd-overlay-close" onClick={() => setShowCadWindow(false)}>âœ•</button>
                    </div>
                    <div className="hd-overlay-body">
                        <CadWindow
                            data={cadData}
            ...
            {showBrowserWindow && (
                <div
                    className="hd-overlay-win"
                    style={{
                        left: elementPositions.browser?.x || '50%',
                        top: elementPositions.browser?.y || '50%',
                        transform: 'translate(-50%,-50%)',
                        width: elementSizes.browser.w,
                        height: elementSizes.browser.h,
                        zIndex: getZIndex('browser'),
                    }}
                    onMouseDown={e => handleMouseDown(e, 'browser')}
                >
                    <div className="hd-overlay-header" data-drag-handle>
                        <span className="hd-overlay-title">WEB AGENT</span>
                        <button className="hd-overlay-close" onClick={() => setShowBrowserWindow(false)}>âœ•</button>
                    </div>
                    <div className="hd-overlay-body">
                        <BrowserWindow
            ...
            {/* â”€â”€ Tool Confirmation Modal â”€â”€ */}
            <ConfirmationPopup

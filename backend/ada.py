...
CHUNK_SIZE = 512  # 32ms per read at 16kHz (was 1024=64ms) — halves pipeline latency
...
config = types.LiveConnectConfig(
    ...
    system_instruction=(
        "Your name is HAPPY — Holistic AI Personal Productivity Yielder. "
        "When anyone asks who you are, what your name is, or about your identity, always reply: "
        "'I'm HAPPY — your AI assistant. I can chat naturally, control your Windows PC, activate your camera, enable hand gestures, send WhatsApp messages, open files, and much more.' "
        "You have a witty, charming, and fun personality. Your creator is Naz, and you address him as 'Sir'. "
        "When answering, respond using complete and concise sentences to keep a quick pacing and keep the conversation flowing.\n\n"
        "## WINDOWS SYSTEM CONTROL\n"
        "You are integrated with a Windows system controller, similar to Apple Siri on macOS. "
        "When the user asks to control their computer — lock screen, shutdown, restart, sleep, adjust volume, adjust brightness, toggle Wi-Fi or Bluetooth, open or close apps, take a screenshot, check battery/CPU/RAM, or search the web — "
        "you MUST call the 'system_control' tool immediately and silently. "
        "Do NOT describe what you're about to do before calling the tool. "
        "After the tool executes and returns a result, respond with a SHORT natural language confirmation of what was done. "
        "Keep the confirmation concise — one sentence only. Do not repeat technical details or JSON. "
        "Examples: 'Your PC is now locked.' / 'Volume set to 40 percent, Sir.' / 'Chrome is open.' "
        "If a command requires admin rights, inform the user politely. "
        "For multi-step commands, call system_control for each step, then give a single combined confirmation. "
        "In normal conversation unrelated to system control, behave exactly like a standard conversational AI.\n\n"
        "## WHATSAPP MESSAGING\n"
        "When the user asks you to send a WhatsApp message, you MUST ask for confirmation first using natural conversational language — "
        "do NOT call the whatsapp_send tool yet. "
        "Ask clearly and concisely, for example: 'Just to confirm — you want me to send \"[message]\" to [contact], right?' "
        "Wait for the user to reply. If they say yes, proceed, approve, send, do it, or anything affirmative — THEN call the whatsapp_send tool immediately. "
        "If they say no, cancel, stop, or anything negative — do NOT call the tool. Reply: 'Understood, message cancelled. Is there anything else I can help you with?' "
        "Never show a system dialog for WhatsApp — handle confirmation entirely through natural conversation."
    ),
    tools=tools,
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, on_frontend_event=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None):
        ...
        self.on_frontend_event = on_frontend_event  # For camera / hand gesture voice control
        ...
        # Echo cancellation flag: True while ADA is playing audio output
        self._ada_playing = False
        self._ada_play_cutoff_time = 0  # Time after playback to keep suppression active
        
        self.permissions = {} # Default Empty (Will treat unset as True)
        
        # on_frontend_event: callback(payload) to emit socket events to the React frontend
        # Used for camera_on/off, hand_gesture_on/off
        from system_agent import SystemAgent
        from whatsapp_agent import WhatsAppAgent
        self.system_agent = SystemAgent(on_frontend_event=on_frontend_event)
    ...
    async def listen_audio(self):
        ...
        while True:
            if self.paused:
                await asyncio.sleep(0.02)  # 20ms - was 100ms, faster unmute response
                continue

            try:
                data = await asyncio.to_thread(self.audio_stream.read, CHUNK_SIZE, **kwargs)
                # 1. Send Audio — skip if ADA is currently playing (echo suppression)
                ada_is_speaking = self._ada_playing or (time.time() < self._ada_play_cutoff_time)
                if self.out_queue and not ada_is_speaking:
                    await self.out_queue.put({"data": data, "mime_type": "audio/pcm"})
                
                # 2. Fast VAD — only compute RMS when we care about state transitions.
                # Skip expensive unpack when ada is speaking (not tracking speech then).
                if not ada_is_speaking:
                    count = len(data) // 2
                    if count > 0:
                        # Vectorised sum-of-squares via struct — ~3x faster than Python list comprehension
                ...
                async for response in turn:
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad", "system_control", "whatsapp_send"]:
                                ...
                                if fc.name == "generate_cad":
                                    ...
                                    target = fc.args.get("target", None)
                                    value  = fc.args.get("value",  None)
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'system_control' intent='{intent}' action='{action}' target='{target}' value='{value}'")
                                    
                                    result_str = await self.system_agent.execute(intent, action, target, value)
                                ...
                                elif fc.name == "whatsapp_send":
                                    contact_name = fc.args.get("contact_name", "")
                                    message      = fc.args.get("message", "")
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'whatsapp_send' contact='{contact_name}'")

                                    # Non-blocking: runs in a background thread via WhatsAppAgent
                                    result_str = self.whatsapp_agent.send_message(contact_name, message)
    ...
    async def play_audio(self):
        ...
        while True:
            ...
            # Mark ADA as actively playing — suppresses mic input to prevent echo
            self._ada_playing = True
            self._ada_play_cutoff_time = time.time() + 0.6  # 600ms tail suppression after last chunk
            if self.on_audio_data:
                self.on_audio_data(bytestream)
            await asyncio.to_thread(stream.write, bytestream)
            # If queue is now empty, ADA finished speaking — lift suppression after cutoff

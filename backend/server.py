...
system_control_authorized = False  # One-time authorization per session
...
async def start_audio(sid, data=None):
    ...
    # Callback to send frontend control events (camera on/off, hand gesture on/off)
    def on_frontend_event(payload):
        print(f"[SERVER] Frontend event: {payload}")
        asyncio.create_task(sio.emit('frontend_control', payload))
    ...
    try:
        ...
        audio_loop = ada.AudioLoop(
            ...
            on_error=on_error,
            on_frontend_event=on_frontend_event,
        ...
        # Emit one-time system authorization request to frontend (if not already authorized)
        if not system_control_authorized:
            await sio.emit('system_auth_request', {
                'actions': [
                    'Volume & audio control',
                    'Lock, shutdown, restart, sleep',
                    'Wi-Fi & Bluetooth toggle',
                    'Open & close applications',
                    'System info queries (battery, CPU, RAM)',
                    'Screenshot & utilities'
                ]
            }, room=sid)
        else:
            # Already authorized this session — apply immediately
            audio_loop.update_permissions({'system_control': False})
...
async def stop_audio(sid):
    global audio_loop, system_control_authorized
    if audio_loop:
        audio_loop.stop() 
        print("Stopping Audio Loop")
        audio_loop = None
        system_control_authorized = False  # Reset auth on disconnect
        await sio.emit('status', {'msg': 'A.D.A Stopped'})

@sio.event
async def authorize_system(sid, data=None):
    """
    Called by the frontend when the user grants one-time system control authorization.
    Sets system_control permission to auto-allow for the rest of this session.
    """
    global system_control_authorized
    granted = (data or {}).get('granted', False)
    
    if granted:
        system_control_authorized = True
        print("[SERVER] System control AUTHORIZED by user (one-time).")
        if audio_loop:
            audio_loop.update_permissions({'system_control': False})  # False = no confirmation needed
        await sio.emit('system_auth_response', {'authorized': True})
    else:
        system_control_authorized = False
        print("[SERVER] System control DENIED by user.")
        if audio_loop:
            audio_loop.update_permissions({'system_control': True})  # True = require confirmation each time
        await sio.emit('system_auth_response', {'authorized': False})
